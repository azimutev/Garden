  ...
  if (auth_header) {
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, (err, decoded) => {
      if (err) {
        ...
        req.user = { id: decoded.userId, is_admin: decoded.is_admin }; 
        next();
      }
    });
  } else {
...
const register = async (req, res) => {
  try {
    let { username, email, password, is_admin } = req.body; // Добави is_admin
    if (!username) {
      ...
      if (user_db) {
        ...
        const new_user = await User.create({
          ...
          is_admin: is_admin || false, // Ако не е предоставено, по подразбиране е false
        });
        // Generate JWT token
        const token = jwt.sign(
          { userId: new_user.id, is_admin: new_user.is_admin }, // Добави is_admin в токена
